...
export async function executeTestCase(params: Params): Promise<ExecuteTestCaseResult> {
  ...
  return traceAgentRun(
    runId,
    {
      "agent.test_case_id": params.testCaseId,
      "agent.trigger": params.trigger ?? (params.trackingUserId ? "interactive" : "scheduled"),
      "agent.mode": params.mode ?? "generate",
    },
    async () => {
      const logs: string[] = [];
      ...
      try {
        if (shouldGenerate) {
          scriptText = await traceAgentStep(
            "script_generation",
            {},
            () => generateTestScript({
            testCase,
        ...
        session = await traceAgentStep(
          "browserbase_session_create",
          {},
          async () => {
            const createdSession = await bb.sessions.create({ projectId: process.env.BROWSERBASE_PROJECT_ID! });
            trace.getActiveSpan()?.setAttribute("browserbase.session_id", createdSession.id);
            return createdSession;
        }
      );
      logs.push(`[SYSTEM] Browserbase session created successfully with ID: ${session.id}`);
      browser = await traceAgentStep(
        "browserbase_connect",
        {},
      ...
      try {
        failureContext = await traceAgentStep(
          "failure_context_fetch",
          {},
          () => fetchFailureContext(testCase, runId)
        );
        const screenshot = page ? await captureScreenshot(page) : null;
        if (screenshot) {
          visionAnalysis = await traceAgentStep(
            "vision_analysis",
            {},
            () => analyzeScreenshot(
            screenshot,
